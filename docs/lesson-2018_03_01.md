...
SELECT SEC_TO_TIME(length*60) AS durationInTime
 FROM sakila.film;
```
Dans l'exemple ci-dessus, on multiplie les minutes par 60 pour avoir des secondes, ensuite on applique la fonction SEC_TO_TIME (exemple : **01:26:00**).




$requeteSuppClient est le template sql
```php
$requeteSuppClient = "delete from clients where id_client = :id";
```
