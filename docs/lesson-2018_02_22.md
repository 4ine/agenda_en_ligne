...
```
`$nbClient` retourne le nombre d'enregistrements total de la table.

Ensuite, nous définissons le nombre d'enregistrements que l'on souhaite afficher par page.
```
$limit = 5;
```
Ces deux informations vont nous permettre de calculer le nombre de pages que nous allons avoir (ceil arrondit au nombre supérieur)
